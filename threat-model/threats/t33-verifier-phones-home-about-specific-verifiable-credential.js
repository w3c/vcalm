(function () {
    var threat = {
        ...
        response: [
            { id: "R30", name: "Use Privacy-Preserving Status List", type: "Reduce", desc: "There are some interactions where contacting the issuer in a privacy-preserving manner upholds the privacy expectations of the holder. For example, contacting the issuer to get revocation status information through a status list that provides group privacy can be acceptable as long as the issuer is not able to single out which verifiable credential is being queried based on the retrieval of the status list. For more information on one such mechanism, see the Bitstring Status List v1.0 specification." },
